...
function toChunks(array, maxChunkSize, Constructor) {
    ...
    for (var i = 0; i < array.length; i += maxChunkSize) {
        var bound = Math.min(maxChunkSize, array.length - lastPos);
        var chunk = new Constructor(array.buffer, lastPos * array.BYTES_PER_ELEMENT, bound);
...
function copyOrWriteArray(length, dataIn, dataOut) {
    if (dataOut === void 0) { dataOut = null; }
    var _dataOut = dataOut === null ? new Float32Array(length) : dataOut;
...
var SRC = /** @class */ (function () {
    ...
    SRC.prototype.full = function (dataIn, dataOut, outLength) {
        if (dataOut === void 0) { dataOut = null; }
        if (outLength === void 0) { outLength = null; }
    ...
    SRC.prototype._chunkAndResample = function (dataIn) {
        ...
        var resampledChunks = [];
        var chunkSize = (this.inputSampleRate / 10) * this.nChannels;
        var chunks = toChunks(dataIn, chunkSize, Float32Array);
        for (var i = 0; i < chunks.length; i++) {
            var resampled = this._resample(this.module.full, chunks[i]);
            accumulatedSize += resampled.length;
            resampledChunks.push(resampled);
        }
        var accumulated = new Float32Array(accumulatedSize);
        var accumulatedIndex = 0;
        for (var i = 0; i < resampledChunks.length; i++) {
            for (var j = 0; j < resampledChunks[i].length; j++) {
    ...
    SRC.prototype._resample = function (resampleFunc, dataIn, dataOut, outLength) {
        ...
        var projectedSize = Math.ceil(dataIn.length * this.ratio);
        ...
        var outputFrames = resampleFunc(dataIn.length, this.nChannels, // ignored by module.full()
